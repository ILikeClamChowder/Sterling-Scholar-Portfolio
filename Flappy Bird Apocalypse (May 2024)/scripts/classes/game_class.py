...
class Game(object):
    def __init__(self):
        ...
        # create sounds
        pg.mixer.Channel(0).set_volume(0.6) # dialogue
        ...
        pg.mixer.Channel(5).set_volume(0.3)  # helicopter
    ...
    def update(self):
        ...
        for bullet in self.enemyBullets:
            if bullet.rect.top > HEIGHT -self.bottomGui.get_height()- 5 or bullet.rect.right < -5 or bullet.rect.bottom < -5 or bullet.rect.left > WIDTH + 5:
                ...
                self.spawnParticle("scripts/assets/sprites/background/point.png", 0,random.randint(int(tileDivide * 20), int(WIDTH - tileDivide * 20)), HEIGHT - 40,[random.randint(-1, 1), random.randint(-5, -1)], 80)
                bullet.kill()

        if not self.trophiesAwarded[0] and self.score>20:
            self.spawnParticle("scripts/assets/sprites/background/bronze.png",0,random.randint(int(tileDivide*20),int(WIDTH-tileDivide*20)),HEIGHT-40,[random.randint(-1,1),random.randint(-3,-1)],140)
            self.trophiesAwarded[0]=True
        elif not self.trophiesAwarded[1] and self.score>75:
            self.spawnParticle("scripts/assets/sprites/background/silver.png",0,random.randint(int(tileDivide*20),int(WIDTH-tileDivide*20)),HEIGHT-40,[random.randint(-1,1),random.randint(-3,-1)],140)
            self.trophiesAwarded[1] = True
        elif not self.trophiesAwarded[2] and self.score>250:
            self.spawnParticle("scripts/assets/sprites/background/gold.png",0,random.randint(int(tileDivide*20),int(WIDTH-tileDivide*20)),HEIGHT-40,[random.randint(-1,1),random.randint(-3,-1)],140)
    ...
    def createBullet(self,num):
        ...
        if num==0: # x axis tube
            ...
            bullet = Bullet(2, WIDTH, random.randint(int(20*tileDivide),int(HEIGHT+20*tileDivide-self.bottomGui.get_height())), [-2,-3])
            bullet.add(self.enemyBullets)
            bullet.add(self.all_sprites)
        elif num==2: # x axis bullet bill
            bullet = Bullet(3, WIDTH, random.randint(0,HEIGHT-self.bottomGui.get_height()), [-2, 0])
            bullet.add(self.enemyBullets)
            bullet.add(self.all_sprites)
            ...
            bullet = Bullet(2, random.randint(0,WIDTH), 0, [random.randint(-1,1), 0],90)
            bullet.add(self.enemyBullets)
            bullet.add(self.all_sprites)
        elif num==5: # y axis bullet bill
            bullet = Bullet(3, random.randint(0,WIDTH), 0, [0, 4],90)
            bullet.add(self.enemyBullets)
            bullet.add(self.all_sprites)
    ...
    def get_game_events(self):
        ...
        for event in events:
            if event.type == pg.QUIT:
                ...
                if event.key == pg.K_a:
                    self.player.playerJump(True,False)
                elif event.key == pg.K_d:
                    self.player.playerJump(False,True)
